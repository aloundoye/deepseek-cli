...
impl AgentEngine {
    ...
    fn new_with_components(
        ...
        if let Some(managed) = codingbuddy_policy::load_managed_settings() {
            ...
            if !servers.is_empty() {
                ...
                tool_host.set_mcp_executor(std::sync::Arc::new(
                    move |server_id, tool_name, args| {
                        ...
                        codingbuddy_mcp::execute_mcp_stdio_request(
                            server,
                            "tools/call",
                            params,
                            1,
    ...
    pub fn cfg_mut(&mut self) -> &mut codingbuddy_core::AppConfig {
        &mut self.cfg
    }
    ...
    fn run_tool_use_loop(&self, prompt: &str, options: &ChatOptions) -> Result<String> {
        ...
        {
            let hooks_config: codingbuddy_hooks::HooksConfig =
        ...
        loop_.set_checkpoint_callback(Arc::new(move |reason, modified_files| {
            if let Ok(mm) = codingbuddy_memory::MemoryManager::new(&ws) {
    ...
    fn build_bootstrap_context(
        ...
        // Budget: ~10% of context window (128K × 0.10 ≈ 12.8K tokens)
        // Reduced from /7 to leave more room for conversation context retention.
        let budget_tokens = context_window_tokens / 10;
        let truncated = truncate_to_token_budget(&packet, budget_tokens);

        vec![ChatMessage::System {
            content: format!(
                "The following is background context about the project. Use it to inform your tool calls and analysis. NEVER reference this context directly in your response — the user already knows their project. Do not mention section headers like \"directory structure\" or \"build manifests\". Just act on the information.\n\n{truncated}"
            ),
        }]
    }
    ...
    #[allow(dead_code)]
    pub(crate) fn record_usage(&self, model: &str, usage: &codingbuddy_core::TokenUsage) {
        self.append_event_best_effort(EventKind::UsageUpdated {
            unit: codingbuddy_core::LlmUnit::Model(model.to_string()),
...
fn build_retriever_callback(
    ...
    let index_path = workspace.join(".codingbuddy").join("vector_index");
    let chunk_config = codingbuddy_local_ml::ChunkConfig {
    ...
    let retriever = match codingbuddy_local_ml::UsearchBackend::new(embeddings.dimension()) {
        ...
        Err(_) => codingbuddy_local_ml::HybridRetriever::new(
            &index_path,
            embeddings,
            None,
            0.7,
...
fn build_privacy_router(
    cfg: &codingbuddy_core::AppConfig,
) -> Option<std::sync::Arc<codingbuddy_local_ml::PrivacyRouter>> {
    ...
    let privacy_config = codingbuddy_local_ml::PrivacyConfig {
        ...
        policy: match cfg.local_ml.privacy.policy.as_str() {
            "block_cloud" => codingbuddy_local_ml::PrivacyPolicy::BlockCloud,
            "local_only_summary" => codingbuddy_local_ml::PrivacyPolicy::LocalOnlySummary,
...
fn should_run_team_orchestration(
    ...
    let value = options
        ...
        .or_else(|| {
            std::env::var("CODINGBUDDY_TEAMMATE_MODE")
                .ok()
...
mod tests {
    ...
    fn shadow_commit_used_in_checkpoint() {
        ...
        let mm = codingbuddy_memory::MemoryManager::new(&workspace).expect("memory manager");
    ...
    fn context_manager_suggest_files_on_real_workspace() {
        ...
        let workspace = std::env::temp_dir().join("codingbuddy_test_ctx_mgr");
        let _ = std::fs::remove_dir_all(&workspace);
