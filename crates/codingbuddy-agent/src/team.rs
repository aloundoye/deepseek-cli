...
use codingbuddy_diff::{GitApplyStrategy, PatchStore};
use serde_json::json;
...
fn require_patch_approval(engine: &AgentEngine, diff: &str, lane: &LaneSpec) -> Result<bool> {
    ...
    match engine.policy.dry_run(&call) {
        codingbuddy_policy::PermissionDryRunResult::Denied(msg) => Err(anyhow::anyhow!(
            "Policy explicitly denied patch apply: {msg}"
        )),
        codingbuddy_policy::PermissionDryRunResult::AutoApproved
        | codingbuddy_policy::PermissionDryRunResult::Allowed => Ok(true),
        codingbuddy_policy::PermissionDryRunResult::NeedsApproval => engine.request_approval(&call),
    }
}
