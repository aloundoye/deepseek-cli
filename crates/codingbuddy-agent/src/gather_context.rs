...
pub fn gather_for_prompt(
    ...
    // Build context using natural language headers (not machine labels) to prevent
    // the model from parroting internal section names back to the user.
    ...
    lines.push(String::new());
    lines.push("Build manifests:".to_string());
    ...
    lines.push(String::new());
    lines.push("README summary:".to_string());
    ...
    if vague_codebase_prompt {
        ...
        lines.push(String::new());
        lines.push("Codebase audit findings:".to_string());
        lines.push("  TODOs and FIXMEs:".to_string());
        ...
        lines.push("  Test surface:".to_string());
        ...
        lines.push("  Risk hotspots:".to_string());
...
mod tests {
    ...
    #[test]
    fn bootstrap_labels_are_natural_language() {
        // The bootstrap packet should use natural language headers, not
        // machine-like uppercase labels that the model might parrot back.
        let forbidden = [
            "AUTO_CONTEXT_BOOTSTRAP",
            "ROOT_TREE_SNAPSHOT",
            "MANIFEST_DISCOVERY",
            "REPO_MAP_SUMMARY",
            "BASELINE_AUDIT",
            ...
            "PROJECT_TYPE:",
            "README_EXCERPT:",
            "AUTO_CONTEXT_BOOTSTRAP_END",
            "USER_REQUEST:",
        ];
        let tmp = tempfile::tempdir().unwrap();
        let ws = tmp.path();

        // Create a minimal workspace so gather_for_prompt produces output
        std::fs::create_dir_all(ws.join(".git")).unwrap();
        std::fs::write(ws.join("Cargo.toml"), "[package]\nname = \"test\"").unwrap();
        std::fs::write(ws.join("README.md"), "# Test project").unwrap();
        std::fs::create_dir_all(ws.join("src")).unwrap();
        std::fs::write(ws.join("src/lib.rs"), "fn main() {}").unwrap();

        let cfg = AppConfig::default();
        let bootstrap = gather_for_prompt(
            ws,
            &cfg,
            "analyze this project",
            ChatMode::Code,
        ...
        assert!(!bootstrap.packet.is_empty(), "packet should not be empty");

        for label in &forbidden {
            assert!(
                !bootstrap.packet.contains(label),
                "bootstrap packet should NOT contain forbidden label '{}'. Full packet:\n{}",
                label,
                bootstrap.packet
            );
        }

        // Verify natural language headers are present
        assert!(bootstrap.packet.contains("This project is located at"));
        assert!(bootstrap.packet.contains("Git status:"));
