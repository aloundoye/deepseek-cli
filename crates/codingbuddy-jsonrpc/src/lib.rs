...
impl IdeRpcHandler {
    pub fn new(workspace: &Path) -> Result<Self> {
        ...
        Ok(Self {
            ...
            include_partial_messages: env_flag_enabled("CODINGBUDDY_INCLUDE_PARTIAL_MESSAGES"),
    ...
    fn handle_prompt_execute(&self, params: Value) -> Result<Value> {
        ...
        self.store.append_event(&codingbuddy_core::EventEnvelope {
            seq_no: seq,
            at: chrono::Utc::now(),
            session_id,
...
mod tests {
    ...
    fn default_handler_initialize() {
        ...
        assert_eq!(result["name"], "codingbuddy-cli");
    }
