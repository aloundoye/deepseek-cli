...
impl HookRuntime {
    ...
    /// requires the agent runtime which is in codingbuddy-agent crate. This
    ...
    fn run_agent_handler(
        ...
        // The main agent loop in codingbuddy-agent reads this and dispatches.
        let context_json = serde_json::to_string(input).unwrap_or_default();
        HookRun {
    ...
    pub fn run_legacy(
        ...
        for path in paths {
            ...
            cmd.stderr(Stdio::null());
            cmd.env("CODINGBUDDY_HOOK_PHASE", &ctx.phase);
            cmd.env(
                "CODINGBUDDY_WORKSPACE",
                ctx.workspace.to_string_lossy().to_string(),
            );
            if let Some(tool_name) = &ctx.tool_name {
                cmd.env("CODINGBUDDY_TOOL_NAME", tool_name);
            }
            if let Some(args) = &ctx.tool_args_json {
                cmd.env("CODINGBUDDY_TOOL_ARGS_JSON", args);
            }
            if let Some(result) = &ctx.tool_result_json {
                cmd.env("CODINGBUDDY_TOOL_RESULT_JSON", result);
            }
...
mod tests {
    ...
    fn legacy_hook_execution() {
        let workspace =
            std::env::temp_dir().join(format!("codingbuddy-hooks-test-{}", uuid::Uuid::now_v7()));
