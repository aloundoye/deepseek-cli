...
impl PolicyEngine {
    ...
    pub fn from_app_config(cfg: &codingbuddy_core::PolicyConfig) -> Self {
        ...
        let mut mapped = PolicyConfig {
            approve_edits: cfg.approve_edits != codingbuddy_core::ApprovalMode::Never,
            approve_bash: cfg.approve_bash != codingbuddy_core::ApprovalMode::Never,
...
fn resolve_team_policy_path() -> Option<std::path::PathBuf> {
    std::env::var("CODINGBUDDY_TEAM_POLICY_PATH")
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
        ...
        .or_else(|| {
            std::env::var("HOME")
                .ok()
                .map(|home| std::path::PathBuf::from(home).join(".codingbuddy/team-policy.json"))
        })
}
...
pub fn managed_settings_path() -> Option<std::path::PathBuf> {
    ...
    if let Ok(path) = std::env::var("CODINGBUDDY_MANAGED_SETTINGS_PATH") {
    ...
    #[cfg(target_os = "macos")]
    {
        let path = std::path::PathBuf::from(
            "/Library/Application Support/CodingBuddyCLI/managed-settings.json",
        );
        return Some(path);
    }
    ...
    #[cfg(target_os = "linux")]
    {
        let path = std::path::PathBuf::from("/etc/codingbuddy-cli/managed-settings.json");
        return Some(path);
    }
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        return Some(std::path::PathBuf::from(
            "C:\\ProgramData\\CodingBuddyCLI\\managed-settings.json",
        ));
    }
...
impl ManagedSettings {
    ...
    pub fn apply_to_config(&self, cfg: &mut codingbuddy_core::AppConfig) {
        // Force permission mode at the config level
        if let Some(ref mode) = self.permission_mode
            && let Ok(parsed) = mode.parse::<codingbuddy_core::PermissionMode>()
        {
        ...
        // (codingbuddy-core's PermissionMode doesn't have Bypass, so this catches
        // the codingbuddy-policy level PermissionMode::BypassPermissions)
        if self.disable_bypass_permissions_mode {
            ...
            if policy_mode == PermissionMode::BypassPermissions {
                cfg.policy.permission_mode = codingbuddy_core::PermissionMode::Ask;
            }
        }
    }
}
...
mod tests {
    ...
    fn sandbox_mode_maps_from_app_config() {
        ...
        unsafe {
            std::env::remove_var("CODINGBUDDY_TEAM_POLICY_PATH");
        }
        let cfg = codingbuddy_core::PolicyConfig {
            sandbox_mode: codingbuddy_core::SandboxMode::ReadOnly,
            ..codingbuddy_core::PolicyConfig::default()
        };
    ...
    fn team_policy_locks_reflect_locked_fields() {
        let _guard = team_policy_env_lock().lock().expect("env lock");
        let dir = std::env::temp_dir().join("codingbuddy-policy-locks");
        ...
        // SAFETY: test-only process-level env mutation.
        unsafe {
            std::env::set_var("CODINGBUDDY_TEAM_POLICY_PATH", &path);
        }
    ...
    fn team_policy_permission_mode_locked_flag() {
        let _guard = team_policy_env_lock().lock().expect("env lock");
        let dir = std::env::temp_dir().join("codingbuddy-policy-pm-lock");
        let _ = fs::create_dir_all(&dir);
        let path = dir.join("team-policy.json");
        fs::write(
            &path,
    ...
    fn managed_overrides_max_turns() {
        ...
        let mut cfg = codingbuddy_core::AppConfig::default();
    ...
    fn symlink_escape_blocked() {
        ...
        let temp = std::env::temp_dir().join("codingbuddy_policy_test_symlink");
