...
use codingbuddy_mcp::McpTransport;
...
#[command(about = "CodingBuddy coding agent", long_about = None)]
#[command(version)]
struct Cli {
    ...
    /// Initialize a new CODINGBUDDY.md in the current directory and exit.
    #[arg(long = "init")]
    init: bool,
...
impl PermissionModeArg {
    fn to_approval_mode(self) -> codingbuddy_core::ApprovalMode {
        match self {
            Self::Ask => codingbuddy_core::ApprovalMode::Ask,
            Self::Always => codingbuddy_core::ApprovalMode::Always,
            Self::Never => codingbuddy_core::ApprovalMode::Never,
        }
    }
}
...
fn run() -> Result<()> {
    ...
    if cli.ide {
        ...
        unsafe { std::env::set_var("CODINGBUDDY_IDE_MODE", "1") };
    }
    if cli.remote {
        // SAFETY: called before any threads are spawned in main()
        unsafe { std::env::set_var("CODINGBUDDY_REMOTE_MODE", "1") };
    }
    if let Some(mode) = cli.teammate_mode.as_deref() {
        // SAFETY: called before any threads are spawned in main()
        unsafe { std::env::set_var("CODINGBUDDY_TEAMMATE_MODE", mode) };
    }
    if !cli.betas.is_empty() {
        // SAFETY: called before any threads are spawned in main()
        unsafe { std::env::set_var("CODINGBUDDY_BETAS", cli.betas.join(",")) };
    }
    if cli.maintenance {
        // SAFETY: called before any threads are spawned in main()
        unsafe { std::env::set_var("CODINGBUDDY_MAINTENANCE_MODE", "1") };
    }
    if cli.include_partial_messages {
        // SAFETY: called before any threads are spawned in main()
        unsafe { std::env::set_var("CODINGBUDDY_INCLUDE_PARTIAL_MESSAGES", "1") };
    }
    ...
    if cli.setting_sources {
        let payload = json!({
            ...
            "managed": codingbuddy_policy::managed_settings_path(),
            "team_policy_env": std::env::var("CODINGBUDDY_TEAM_POLICY_PATH").ok(),
            "managed_env": std::env::var("CODINGBUDDY_MANAGED_SETTINGS_PATH").ok(),
        });
        if cli.json {
            print_json(&payload)?;
        } else {
    ...
    // Handle --init: create CODINGBUDDY.md and exit
    if cli.init {
    ...
    if let Some(ref _categories) = cli.debug {
        // SAFETY: called before any threads are spawned in main()
        unsafe { std::env::set_var("CODINGBUDDY_DEBUG", "1") };
    }

    // Handle --settings: load settings from path or inline JSON
    if let Some(ref settings_arg) = cli.settings {
        // SAFETY: called before any threads are spawned in main()
        unsafe { std::env::set_var("CODINGBUDDY_SETTINGS", settings_arg) };
    }
    ...
    match command {
        Commands::Chat(args) => {
            // ChatArgs.model overrides the global --model if specified
...
mod tests {
    ...
    fn native_host_command_parses() {
        let cli = Cli::try_parse_from(["codingbuddy", "native-host"]).expect("parse native-host");
    ...
    fn setup_command_parses() {
        let cli = Cli::try_parse_from(["codingbuddy", "setup"]).expect("parse setup");
        assert!(matches!(cli.command, Some(Commands::Setup(_))));
    }

    #[test]
    fn setup_local_ml_flag_parses() {
        let cli = Cli::try_parse_from(["codingbuddy", "setup", "--local-ml"])
    ...
    fn setup_status_flag_parses() {
        let cli = Cli::try_parse_from(["codingbuddy", "setup", "--status"])
            .expect("parse setup --status");
        if let Some(Commands::Setup(args)) = cli.command {
            assert!(args.status);
            assert!(!args.local_ml);
        } else {
