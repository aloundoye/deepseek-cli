use anyhow::Result;
use codingbuddy_subagent::{CustomAgentDef, SubagentConfig, load_agent_defs};
use serde_json::json;
use std::path::Path;
    ...
    match cmd {
        AgentsCmd::List => {
            ...
            if json_mode {
                ...
                println!(
                    "Create .md files in .codingbuddy/agents/ or ~/.codingbuddy/agents/ with YAML frontmatter."
                );
            } else {
        ...
        AgentsCmd::Create { name } => {
            let agents_dir = cwd.join(".codingbuddy/agents");
