...
pub(crate) fn run_index(cwd: &Path, cmd: IndexCmd, json_mode: bool) -> Result<()> {
    ...
    match cmd {
        ...
        IndexCmd::Doctor => {
            let status = service.status()?;
            let index_path = cwd.join(".codingbuddy").join("index");
    ...
    match cmd {
        ...
        PermissionsCmd::Set(args) => {
            ...
            if let Some(mode) = args.sandbox_mode {
                ...
                let parsed: codingbuddy_core::SandboxMode =
...
pub(crate) fn run_clean(cwd: &Path, args: CleanArgs, json_mode: bool) -> Result<()> {
    let candidates = vec![
        cwd.join(".codingbuddy/patches"),
        cwd.join(".codingbuddy/observe.log"),
        cwd.join(".codingbuddy/index/tantivy"),
    ];
