...
use deepseek_jsonrpc::{JsonRpcRequest, JsonRpcResponse, RpcHandler};
...
pub(crate) fn run_native_host(cwd: &Path, args: NativeHostArgs) -> Result<()> {
    ...
    let handler = deepseek_jsonrpc::IdeRpcHandler::new(&workspace_root)?;
    let stdin = io::stdin();
...
fn handle_native_request(handler: &impl RpcHandler, raw: &[u8]) -> (JsonRpcResponse, bool) {
    let request = match parse_native_request(raw) {
        ...
        Err(err) => {
            return (
                JsonRpcResponse::error(Value::Null, deepseek_jsonrpc::ERR_PARSE, err.to_string()),
                false,
            );
        }
    };
    ...
    match handler.handle(&request.method, request.params) {
        ...
        Err(err) => (
            JsonRpcResponse::error(request.id, deepseek_jsonrpc::ERR_INTERNAL, err.to_string()),
            false,
        ),
    }
}
