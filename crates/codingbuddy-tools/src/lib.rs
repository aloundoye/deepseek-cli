...
mod tests {
    ...
    fn temp_host() -> (PathBuf, LocalToolHost) {
        let workspace =
            std::env::temp_dir().join(format!("codingbuddy-tools-test-{}", Uuid::now_v7()));
    ...
    fn fs_read_emits_visual_artifact_event_when_enabled() {
        let workspace =
            std::env::temp_dir().join(format!("codingbuddy-tools-vis-{}", Uuid::now_v7()));
        fs::create_dir_all(&workspace).expect("workspace");
    ...
    fn read_only_sandbox_blocks_mutating_bash_commands() {
        let workspace =
            std::env::temp_dir().join(format!("codingbuddy-tools-ro-{}", Uuid::now_v7()));
        fs::create_dir_all(&workspace).expect("workspace");
        let mut cfg = AppConfig::default();
    ...
    fn workspace_write_sandbox_blocks_absolute_outside_paths() {
        let workspace =
            std::env::temp_dir().join(format!("codingbuddy-tools-ww-{}", Uuid::now_v7()));
        fs::create_dir_all(&workspace).expect("workspace");
        let outside =
            std::env::temp_dir().join(format!("codingbuddy-outside-{}.txt", Uuid::now_v7()));
    ...
    fn diagnostics_check_detects_rust_project() {
        let workspace =
            std::env::temp_dir().join(format!("codingbuddy-tools-diag-{}", Uuid::now_v7()));
        fs::create_dir_all(&workspace).expect("workspace");
