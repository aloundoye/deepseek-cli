...
impl PluginManager {
    ...
    pub fn install(&self, source: &Path) -> Result<PluginInfo> {
        if !is_plugin_root(source) {
            return Err(anyhow!(
                "invalid plugin root: missing .codingbuddy-plugin/plugin.json"
            ));
        }
...
mod tests {
    ...
    fn discovers_plugin_layout() {
        let workspace =
            std::env::temp_dir().join(format!("codingbuddy-plugin-test-{}", uuid::Uuid::now_v7()));
        fs::create_dir_all(workspace.join("plugins/demo/.codingbuddy-plugin"))
            .expect("create dirs");
        fs::write(
            workspace.join("plugins/demo/.codingbuddy-plugin/plugin.json"),
    ...
    fn renders_plugin_command_prompt_with_input_substitution() {
        ...
        fs::create_dir_all(plugin.join(".codingbuddy-plugin")).expect("create dirs");
        fs::create_dir_all(plugin.join("commands")).expect("commands dir");
        fs::write(
            plugin.join(".codingbuddy-plugin/plugin.json"),
            r#"{"id":"demo","name":"Demo","version":"0.1.0"}"#,
        )
        .expect("manifest");
        fs::write(
