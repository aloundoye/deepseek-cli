...
pub struct RewindResult {
    ...
    pub action: codingbuddy_core::RewindAction,
...
impl MemoryManager {
    ...
    pub fn memory_path(&self) -> PathBuf {
        self.workspace.join("CODINGBUDDY.md")
    }
    ...
    pub fn global_memory_path() -> Option<PathBuf> {
        ...
        Some(PathBuf::from(home).join(".codingbuddy/CODINGBUDDY.md"))
    }
    ...
    pub fn read_combined_memory(&self) -> Result<String> {
        ...
        // 1. Global memory: ~/.codingbuddy/CODINGBUDDY.md
        ...
        // 2. Hierarchical CODINGBUDDY.md files (root → cwd).
        let hierarchical = load_hierarchical_memory(&self.workspace);
        for (path, content) in &hierarchical {
            // Skip the workspace-level CODINGBUDDY.md — we already load it below.
        ...
        // 3. Project-level CODINGBUDDY.md (workspace root).
    ...
    pub fn rewind_with_action(
        &self,
        checkpoint_id: Uuid,
        action: codingbuddy_core::RewindAction,
    ) -> Result<RewindResult> {
        use codingbuddy_core::RewindAction;
        let checkpoint = self
            .store
            .load_checkpoint(checkpoint_id)?
            .ok_or_else(|| anyhow!("checkpoint not found: {checkpoint_id}"))?;
    ...
    /// Create a shadow commit on hidden ref `refs/codingbuddy-shadow/<id>`.
    ...
    pub fn create_shadow_commit(&self, reason: &str) -> Result<ShadowCommit> {
        ...
        let ref_name = format!("refs/codingbuddy-shadow/{commit_id}");
    ...
    pub fn restore_shadow_commit(&self, shadow_id: Uuid) -> Result<()> {
        let ref_name = format!("refs/codingbuddy-shadow/{shadow_id}");
        let workspace = &self.workspace;
    ...
    pub fn export_transcript(
        ...
        let body = match format {
            ...
            ExportFormat::Markdown => {
                let mut md = String::from("# CodingBuddy Transcript Export\n\n");
...
/// A loaded rule file from `.codingbuddy/rules/` or `~/.codingbuddy/rules/`.
#[derive(Debug, Clone)]
pub struct RuleFile {
    /// Source path of the rule file.
    pub path: PathBuf,
    /// Whether this is a user-level rule (from ~/.codingbuddy/rules/).
    pub user_level: bool,
...
pub fn load_rules(workspace: &Path) -> Vec<RuleFile> {
    ...
    // User-level rules: ~/.codingbuddy/rules/*.md
    if let Some(home) = std::env::var("HOME")
        .ok()
        .or_else(|| std::env::var("USERPROFILE").ok())
    {
        let user_rules_dir = PathBuf::from(&home).join(".codingbuddy/rules");
        if user_rules_dir.is_dir() {
            load_rules_from_dir(&user_rules_dir, true, &mut rules);
        }
    }

    // Project-level rules: .codingbuddy/rules/*.md
    let project_rules_dir = workspace.join(".codingbuddy/rules");
...
// ── Hierarchical CODINGBUDDY.md Loading ────────────────────────────────────────

/// Load CODINGBUDDY.md files hierarchically from cwd up to filesystem root.
...
pub fn load_hierarchical_memory(workspace: &Path) -> Vec<(PathBuf, String)> {
    ...
    loop {
        let codingbuddy_md = dir.join("CODINGBUDDY.md");
        if codingbuddy_md.exists()
            && let Ok(content) = fs::read_to_string(&codingbuddy_md)
        {
...
mod tests {
    ...
    fn auto_memory_observations_are_persisted_and_loaded() {
        let workspace =
            std::env::temp_dir().join(format!("codingbuddy-memory-auto-{}", Uuid::now_v7()));
        let home = std::env::temp_dir().join(format!("codingbuddy-memory-home-{}", Uuid::now_v7()));
        fs::create_dir_all(&workspace).expect("workspace");
    ...
    fn loads_rules_from_project_directory() {
        let workspace =
            std::env::temp_dir().join(format!("codingbuddy-rules-test-{}", Uuid::now_v7()));
        let rules_dir = workspace.join(".codingbuddy/rules");
    ...
    fn process_imports_resolves_files() {
        let dir = std::env::temp_dir().join(format!("codingbuddy-import-test-{}", Uuid::now_v7()));
    ...
    fn process_imports_handles_missing_file() {
        let dir =
            std::env::temp_dir().join(format!("codingbuddy-import-missing-{}", Uuid::now_v7()));
        fs::create_dir_all(&dir).expect("dir");
    ...
    fn process_imports_respects_max_depth() {
        let dir = std::env::temp_dir().join(format!("codingbuddy-import-depth-{}", Uuid::now_v7()));
        fs::create_dir_all(&dir).expect("dir");
    ...
    fn hierarchical_memory_loads_upward() {
        let base = std::env::temp_dir().join(format!("codingbuddy-hier-{}", Uuid::now_v7()));
        let child = base.join("project/sub");
        fs::create_dir_all(&child).expect("dirs");

        fs::write(base.join("CODINGBUDDY.md"), "Root memory").expect("root");
        fs::write(base.join("project/CODINGBUDDY.md"), "Project memory").expect("project");
        fs::write(child.join("CODINGBUDDY.md"), "Sub memory").expect("sub");
    ...
    fn cleanup_deletes_expired_checkpoints() {
        let workspace =
            std::env::temp_dir().join(format!("codingbuddy-mem-cleanup-{}", Uuid::now_v7()));
        fs::create_dir_all(&workspace).expect("workspace");
        ...
        let store = codingbuddy_store::Store::new(&workspace).expect("store");
    ...
    fn cleanup_keeps_recent_checkpoints() {
        let workspace =
            std::env::temp_dir().join(format!("codingbuddy-mem-keep-{}", Uuid::now_v7()));
        fs::create_dir_all(&workspace).expect("workspace");
    ...
    fn cleanup_returns_count() {
        let workspace =
            std::env::temp_dir().join(format!("codingbuddy-mem-count-{}", Uuid::now_v7()));
        fs::create_dir_all(&workspace).expect("workspace");
    ...
    fn rewind_action_restore_code_and_convo() {
        let workspace =
            std::env::temp_dir().join(format!("codingbuddy-mem-rw-both-{}", Uuid::now_v7()));
        fs::create_dir_all(&workspace).expect("workspace");
    ...
    fn rewind_action_code_only() {
        let workspace =
            std::env::temp_dir().join(format!("codingbuddy-mem-rw-code-{}", Uuid::now_v7()));
        fs::create_dir_all(&workspace).expect("workspace");
    ...
    fn rewind_action_convo_only() {
        let workspace =
            std::env::temp_dir().join(format!("codingbuddy-mem-rw-convo-{}", Uuid::now_v7()));
        fs::create_dir_all(&workspace).expect("workspace");
    ...
    fn rewind_action_cancel() {
        let workspace =
            std::env::temp_dir().join(format!("codingbuddy-mem-rw-cancel-{}", Uuid::now_v7()));
        fs::create_dir_all(&workspace).expect("workspace");
        ...
        let result = manager
            .rewind_with_action(cp.checkpoint_id, codingbuddy_core::RewindAction::Cancel)
            .expect("rewind");
    ...
    fn make_test_workspace(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("codingbuddy-snap-{name}-{}", Uuid::now_v7()));
