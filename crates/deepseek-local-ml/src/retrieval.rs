...
impl HybridRetriever {
    ...
    pub fn build_index(&mut self, workspace: &Path) -> Result<IndexBuildReport> {
        ...
        if let Some(ref tantivy) = self.tantivy_index {
            let chunk_entries: Vec<deepseek_index::ChunkEntry> = chunks
                .iter()
                .map(|c| deepseek_index::ChunkEntry {
                    chunk_id: c.id.clone(),
...
mod tests {
    ...
    fn hybrid_search_with_bm25_contributes_score() {
        ...
        // Build tantivy index
        let session = deepseek_core::Session {
        ...
        let tantivy = deepseek_index::IndexService::new(&workspace).unwrap();
