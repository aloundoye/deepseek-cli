//! Smart context management for CodingBuddy
//!
...
impl ContextManager {
    ...
    pub fn analyze_workspace(&mut self) -> Result<()> {
        ...
        let walker = WalkBuilder::new(&self.workspace_root)
            .hidden(false)
            .add_custom_ignore_filename(".codingbuddyignore")
            .build();
...
mod tests {
    ...
    fn deepseekignore_excludes_files() -> Result<()> {
        ...
        // Create .codingbuddyignore excluding "secret/"
        fs::write(root.join(".codingbuddyignore"), "secret/\n")?;
        ...
        assert!(
            ...
            "secret/ should be excluded by .codingbuddyignore, got: {:?}",
            hidden_paths
        );
